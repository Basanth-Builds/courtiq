...
export default function AdminPage() {
  ...
  const [creatingMatchForPool, setCreatingMatchForPool] = useState<string | null>(null)
  ...
  const [deletingTeam, setDeletingTeam] = useState<{ matchId: string; team: 'team1' | 'team2'; teamName: string; opponentName: string } | null>(null)
  ...
  const handleCreateMatch = async (poolId: string) => {
    if (!newMatchData.team1 || !newMatchData.team2) {
      showMessage('error', 'Please enter both team names')
      return
    }

    try {
      const res = await fetch('/api/scores/match/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          poolId,
          ...
          courtNumber: newMatchData.courtNumber,
          status: 'SCHEDULED',
        }),
      })

      if (res.ok) {
        setCreatingMatchForPool(null)
        setNewMatchData({ team1: '', team2: '', courtNumber: undefined })
        showMessage('success', 'Match created successfully')
        refresh()
      } else {
        const error = await res.json()
        showMessage('error', error.error || 'Failed to create match')
      }
    } catch (error) {
  ...
  const handleDeleteTeam = async () => {
    ...
    try {
      const res = await fetch(`/api/scores/match/${deletingTeam.matchId}`, {
        method: 'DELETE',
      })

      if (res.ok) {
        showMessage('success', `Team and match deleted successfully`)
        setDeletingTeam(null)
        refresh()
      } else {
        const error = await res.json()
        showMessage('error', error.error || 'Failed to delete team')
      }
    } catch (error) {
  ...
  if (!tournament) return <div className="min-h-screen flex items-center justify-center">No tournament data</div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0F1117] via-[#1A1D2E] to-[#0F1117] text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        {activeTab === 'courts' && (
          <CourtManagement matches={allMatches} onRefresh={refresh} />
        )}
                      ...
                      return (
                        <div key={pool.id}>
                          ...
                          {addingTeamToPool === pool.id ? (
                            ...
                            <div className="flex gap-2 mb-4">
                              <button
                                ...
                                className="px-3 py-1.5 bg-blue-500/20 text-blue-400 border border-blue-500/30 rounded hover:bg-blue-500/30 text-xs font-medium"
                              >
                                + Add Team to Pool
                              </button>
                              <button
                                onClick={() => setCreatingMatchForPool(pool.id)}
                                className="px-3 py-1.5 bg-purple-500/20 text-purple-400 border border-purple-500/30 rounded hover:bg-purple-500/30 text-xs font-medium flex items-center gap-1"
                              >
                          ...
                          {creatingMatchForPool === pool.id && (
                            <div className="mb-4 p-4 bg-[#1E2030]/50 border border-purple-500/20 rounded-lg">
                              <h4 className="text-sm font-medium text-white/70 mb-3">Create New Match</h4>
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                                <input
                                  ...
                                  className="px-3 py-2 bg-[#1A1D2E] border border-white/10 rounded-lg text-white placeholder:text-white/30 text-sm"
                                />
                                <input
                                  type="text"
                                  placeholder="Team 2"
                                ...
                                <input
                                  type="number"
                                  placeholder="Court #"
                                  value={newMatchData.courtNumber || ''}
                                  onChange={(e) => setNewMatchData({ ...newMatchData, courtNumber: e.target.value ? Number(e.target.value) : undefined })}
                                  className="px-3 py-2 bg-[#1A1D2E] border border-white/10 rounded-lg text-white placeholder:text-white/30 text-sm"
                                />
                              </div>
                              <div className="flex gap-2">
                                <button
                                  onClick={() => handleCreateMatch(pool.id)}
                                  className="px-4 py-2 bg-gradient-to-r from-[#A8D634] to-[#8BC929] text-[#0F1117] rounded-lg 
                                    hover:shadow-lg hover:shadow-[#A8D634]/25 transition-all font-medium text-xs"
                                >
                                  Create
                                </button>
                                <button
                                  ...
                                  className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg transition-all text-xs"
                                >
                                  Cancel
                                </button>
                              </div>
                                ...
                                <tbody className="divide-y divide-white/8">
                                  {pool.matches.map((match) => (
                                    <MatchRow
                                      ...
                                      onDeleteTeam={(team: 'team1' | 'team2') => setDeletingTeam({ 
                                        matchId: match.id, 
                                        team, 
                                        teamName: team === 'team1' ? match.team1 : match.team2,
                                        opponentName: team === 'team1' ? match.team2 : match.team1
                                      })}
                                    />
                                  ))}
                                </tbody>
      ...
      {deletingTeam && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1A1D2E] rounded-2xl border border-white/10 max-w-md w-full p-6 shadow-2xl">
            ...
            <p className="text-white/60 mb-4">
              You are about to delete team:<br />
              <span className="font-semibold text-white">{deletingTeam.teamName}</span>
              <br /><br />
              ⚠️ <strong className="text-orange-400">Warning:</strong> Deleting a team will delete the <strong>entire match</strong>:<br />
              <span className="font-semibold text-white">{deletingTeam.teamName} vs {deletingTeam.opponentName}</span>
              <br /><br />
              This will also delete all associated game data and cannot be undone.
            </p>
            <div className="flex gap-3">
...
function MatchRow({
  ...
  onDelete: () => void
  onDeleteTeam: (team: 'team1' | 'team2') => void
}) {
      ...
      <td className="px-4 py-3">
        {isEditingTeam1 ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => onDeleteTeam('team1')}
              className="opacity-0 group-hover:opacity-100 px-1.5 py-0.5 bg-red-500/10 hover:bg-red-500/20 rounded text-xs text-red-400"
              title="Delete this team (deletes entire match)"
            >
